...
export default class Scene7_3A extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('empty-hat', 'assets/Images/7_Level4/sprite/envelope/empty-hat.png')
    }
    ...
    create() {
        ...
        tileBorder.setScale(0.93, 0.88);


        //Tile text.
        this.heading = this.add.rexBBCodeText(960, 265,
            `[b]FACT 1[/b]`,
            { fontFamily: "Arial", fontSize: "82px", color: '#783a0d', align: 'center' }).setOrigin(0.5);
        // Dealing with text quality.
        this.heading.scale = 0.5

        // Wave texture
        const wave = this.add.sprite(1540, 255, 'wave').setOrigin(0.5)

        // The Hat is empty before 
        const emptyHat = this.add.sprite(520, 540, 'empty-hat').setOrigin(0.5);
        emptyHat.setScale(0.6);
        // Rabbit in the hat
        const rabbitInHat = this.add.sprite(520, 540, 'rabbit-in-hat').setOrigin(0.5);
        rabbitInHat.scale = 0.6;

        /** animation for rabbit to fade in at the start of the scene*/
        rabbitInHat.setAlpha(0);
        this.tweens.add({
            targets: rabbitInHat,
            alpha: 1,
            delay: 700,
        ...
        // Text
        this.text = this.add.text(1217, 610,
