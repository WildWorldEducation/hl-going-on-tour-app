...
app.post("/api/login-attempt", (req, res, next) => {
  ...
  let sqlQuery1 =
    "SELECT * FROM healthy_lifestyles.users WHERE healthy_lifestyles.users.username = '" +
    req.body.username +
    "' AND healthy_lifestyles.users.password = '" +
    req.body.password +
    "';";
...
app.get("/api/user-progress/:id", function (req, res, next) {
  ...
  if (req.session.userName) {
    let sqlQuery =
      ...
      WHERE healthy_lifestyles.users.id = ` +
      req.params.id +
      `;`;
...
app.put("/api/unlock-module", function (req, res, next) {
  let sqlQuery =
    "UPDATE users SET module_unlocked='" +
    req.body.moduleNum +
    "' WHERE id = " +
    req.session.userId +
    ";";
